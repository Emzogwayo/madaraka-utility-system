...
export default function ResidentDashboard() {
  ...
  // ==========================================
  const [user, setUser] = useState(null); // Stores the logged-in resident
  const [reports, setReports] = useState([]); // Stores the tickets fetched from Firebase
  const [loadingTickets, setLoadingTickets] = useState(true); // Controls the loading spinner
  
  // UX STATE: Controls which tab is currently visible without refreshing the page
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        // QUERY: Fetch only the tickets where the residentId matches the logged-in user's ID
        const q = query(
          collection(db, "tickets"), 
          where("residentId", "==", currentUser.uid)
        );

        // ONSNAPSHOT: Creates a live WebSocket connection to Firebase. 
        // If a dispatcher updates a ticket, this instantly updates the resident's screen.
        const unsubscribeTickets = onSnapshot(q, (snapshot) => {
          ...
          // SORTING: Order tickets so the newest ones appear at the top
        ...
        // Cleanup function to prevent memory leaks when the user leaves the page
  ...
  // LIFECYCLE END: Resident confirms the issue is fixed. Moves status to 'Closed'.
  ...
  // LIFECYCLE REJECTION: Resident says issue isn't fixed. Kicks ticket back to 'Pending'.
  ...
  // LIFECYCLE ESCALATION: Resident flags a delayed ticket. Alerts the Admin.
  const handleEscalateTicket = async (ticketId) => {
    try {
      const ticketRef = doc(db, "tickets", ticketId);
      await updateDoc(ticketRef, { 
        status: "Escalated",
        escalatedAt: serverTimestamp() // Audit trail for Eniola's logs next week
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-900">
      ...
      <aside className="w-full md:w-64 bg-white border-r border-slate-200 flex flex-col">
        ...
        {/* Tab routing utilizing the setActiveTab state */}
      ...
      <main className="flex-1 p-6 lg:p-10">
        ...
        {activeTab === "dashboard" && (
          <div className="max-w-4xl">
            ...
            {/* UPGRADED: Premium Gradient Call to Action */}
            ...
            <div>
              <div className="flex items-center justify-between mb-4 px-1">
                ...
                {/* Clicking this button magically switches the user to the Reports tab! */}
                <button 
                  onClick={() => setActiveTab("reports")}
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="divide-y divide-slate-100">
                  {reports.length === 0 ? (
                    ...
                    // slice(0, 3) ensures we ONLY show the top 3 most recent tickets here
              ...
              <div className="divide-y divide-slate-100">
                {loadingTickets ? (
                  ...
                  reports.map((report) => {
                    ...
                    // ----------------------------------------------------
                    // TIME CALCULATION LOGIC FOR MANUAL ESCALATION
                    // ----------------------------------------------------
                    const now = new Date();
                    const ticketDate = report.createdAt ? report.createdAt.toDate() : now;
                    // Divide by milliseconds, seconds, and minutes to get total hours passed
                    const hoursDifference = (now - ticketDate) / (1000 * 60 * 60);
                    
                    // BUSINESS LOGIC: A ticket is overdue if it has been Pending for > 48 hours.
                      ...
                      <div key={report.id} className="p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4 hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex items-start gap-4 flex-1">
                          ...
                          <div className="flex-1 w-full">
                            ...
                            {/* Display Technician Details if Dispatched */}
